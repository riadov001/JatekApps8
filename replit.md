...
- `pnpm --filter @workspace/mockup-sandbox run dev` — run the Admin Dashboard (port 8081, serves at `/admin`)
...
- **mockup-sandbox repurposed**: The Replit `design` artifact (originally a canvas sandbox) now hosts the Admin Dashboard at `/admin`. Artifact kind cannot be changed after creation.
- **23-table Drizzle schema**: Copied from Jatekapoli reference app. Push with `pnpm --filter @workspace/db run push` (dev only).

## Product

- **Admin Dashboard** (`/admin`): Full web app for staff to manage orders, restaurants, products, categories, drivers, customers, staff roles, promotions, wallets, notifications, reviews, and reports. Login with `admin@jatek.ma`.
...
- Three mobile apps (driver, client, merchant) deferred to future sessions — free-tier single-artifact limit applies
